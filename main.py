...
class Compliment():
    def __init__(self):
        self.text = req.get("https://complimentr.com"
                            "/api").json()["compliment"]

    def get_compliment(self):
        translator = Translator()
        str_to_return = translator.translate(self.text,
                                             src='en', dest='ru').text
...
class Weather():
    def __init__(self, lat, lon):
        ...
        self.weather = req.get("https://api.openweathermap.org"
    ...
    def get_weather(self):
        translator = Translator()
        str_to_return = "Погода: " \
                        + translator.translate(self.weather.json() ["weather"]
                        [0] ["description"],
                        src='en', dest='ru').text + "\n"
        str_to_return += "Tемпература: " \
                         + str(self.weather.json()["main"]["temp"] - 273.15) \
                         + " градусов Цельсия\n"
        str_to_return += "Cкорость ветра: " \
                         + str(self.weather.json()["wind"]["speed"]) \
                         + " м/c"
        return str_to_return
...
class Recipe:
    def __init__(self):
        recipe = req.get("https://www.themealdb.com/api/json/v1/1/"
                         "random.php").json()['meals'][0]
        self.name = recipe ['strMeal']
        self.instructions = recipe['strInstructions']
        self.ingredients = ''
        for i in range(1, 21):
            if recipe ["strIngredient" + str(i)] != '' \
                    and recipe ["strMeasure" + str(i)] != '':
                self.ingredients += recipe ["strIngredient" + str(i)] \
                                    + "   " + recipe ["strMeasure" + str(i)] \
                                    + "\n"

    def get_recipe(self):
        translator = Translator()
        str_to_return = translator.translate(self.name,src='en',
                                             dest='ru').text + "\n\n"
        str_to_return += translator.translate(self.instructions, src='en',
                                              dest='ru').text + "\n\n"
        str_to_return += translator.translate(self.ingredients, src='en',
                                              dest='ru').text + "\n\n"
        return str_to_return
...
class Anecdote:
    def __init__(self):
        self.text = req.get("http://rzhunemogu.ru"
                            "/RandJSON.aspx?CType=1").text[12:-3]
...
def startBot(message):
    first_mess = f"{message.from_user.first_name}, привет!\n" \
                 f"Я - супер крутой телеграм бот для домохозяек," \
...
def func(message):
    if message.text == "Анекдот":
        ...
        button_compliment = types.InlineKeyboardButton(
    ...
    if message.text == "Погода в Омске":
        ...
        button_kirovsky = types.InlineKeyboardButton(text='Кировский',
                                                     callback_data='kirovsky')
        markup.add(button_kirovsky)
        button_sovetsky = types.InlineKeyboardButton(text='Советский',
                                                     callback_data='sovetsky')
        markup.add(button_sovetsky)
        button_central = types.InlineKeyboardButton(text='Центральный',
                                                    callback_data='central')
        markup.add(button_central)
        button_oktyabrsky = types.InlineKeyboardButton(text='Октябрьский',
                                                       callback_data='oktyabrsky')
        markup.add(button_oktyabrsky)
        button_leninsky = types.InlineKeyboardButton(text='Ленинский',
                                                     callback_data='leninsky')
